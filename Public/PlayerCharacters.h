...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FSkillHasActivated, APlayerCharacters*, ActivatingActor,
                                             class UPlayerSkill*, ActivatedSkill);
...
protected:
    ...
    void UnlockNewSkill(class UPlayerSkill* Skill);

public:
    virtual void GetOwnedGameplayTags(FGameplayTagContainer& OwnedTags) const override
    {
        if (AbilitySystem)
            OwnedTags = AbilitySystem->GetOwnedGameplayTags();
        return;
    }
    
    bool CanReceiveInput() const
    {
        return !AbilitySystem->HasAnyMatchingGameplayTags(InputBlockingTags) && !IsImmobile();
    }
...
public:
    ...
    UFUNCTION()
    void ReceiveXP(float XPAmount);

    UFUNCTION(BlueprintCallable,meta=(ToolTip="Should only reasonably be called for debugging purpoess"))
    void ReceiveSkillNode(USkillTreeNodeData* SkillTreeNode, FName SkillName);
    
    void ActivateSkillNode(USkillTreeNodeData* SkillTreeNode);
    ...
    UFUNCTION()
    FEquipmentToSave GetEquipmentSaveData() const;

    UFUNCTION()
    void LoadEquipmentFromSaveData(FEquipmentToSave& SaveData);
    
    FName GetIdentifyingTag() const
    {
...
protected:
    ...
    UPROPERTY(VisibleAnywhere,BlueprintReadWrite, Category = "Enhancement Component")
    class UCharacterGrowthComponent* GrowthComponent;
